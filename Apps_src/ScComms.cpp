...
void ScComms::handleExternalConnection(int fd) {
    ...
    if (spacecraft->isConnected()) {
        ...
        fprintf(getAppl()->getLogFileID(), "Error: Spacecraft is already connected to OSPRE, cannot have two spacecraft connections\n");
    }
...
void ScComms::handleOSPREStatus(OSPREStatus* msg, ServiceInternal* service) {
    ...
    externalOspreStatusMessage->print(logFile);
