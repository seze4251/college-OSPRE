function [error_range] = Angles_Position_Error(alpha, beta, sigma_mean)
...
for i = 1:vec_length
    ...
    r_SC_M(:,i) = (pos_Moon_ECI(:,i)-pos_ECI(:,i))/norm(pos_Moon_ECI(:,i)-pos_ECI(:,i));       % Perfect SC attitude unit vector Moon
end
...
for i = 1:length(sigma_mean)
    for j = 1:vec_length
        pos_Calc(:,j,i) = Position_From_Angles_Vec(pos_Moon_ECI(:,j), r_SC_E(:,j), r_SC_M(:,j), alpha+sigma_mean(i), beta+sigma_mean(i));
